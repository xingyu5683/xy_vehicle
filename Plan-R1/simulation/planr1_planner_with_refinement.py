...
class PlanR1PlannerWithRefinement(PDMClosedPlanner):
    ...
    def __init__(
        ...
        """
        super(PlanR1PlannerWithRefinement, self).__init__(
            trajectory_sampling,
            proposal_sampling,
            idm_policies,
            lateral_offsets,
        ...
        self._planr1_planner = planr1_planner
    ...
    def initialize(self, initialization: PlannerInitialization) -> None:
        ...
        self._planr1_planner.initialize(initialization)
        # nuboard debug info
    ...
    def _get_closed_loop_trajectory(
        ...
        if str_pred_states is None:
            trajectory = self._planr1_planner.compute_planner_trajectory(current_input)
            target_timestamps = np.linspace(start=trajectory.start_time.time_us, stop=trajectory.end_time.time_us, num=81)
            str_pred_states = [trajectory.get_state_at_time(TimePoint(t)) for t in target_timestamps if trajectory.is_in_range(TimePoint(t))]
