...
from diffusion_planner.data_process.data_processor_with_nextstate import DataProcessor
...
def get_filter_parameters(num_scenarios_per_type=None, limit_total_scenarios=None, shuffle=True, scenario_tokens=None, log_names=None):
    ...
    expand_scenarios = False             # Whether to expand multi-sample scenarios to multiple single-sample scenarios
                                        # Set to False to keep full time sequences for next state processing
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Data Processing with Next State')
    ...
    with open('./nuplan_train_test.json', "r", encoding="utf-8") as file:
        log_names = json.load(file)

    map_version = "nuplan-maps-v1.0"    
    builder = NuPlanScenarioBuilder(args.data_path, args.map_path, sensor_root, db_files, map_version)#初始化 NuPlanScenarioBuilder，用于从数据集中加载场景 扫描 data_path 目录下的所有 .db 数据库文件
    scenario_filter = ScenarioFilter(*get_filter_parameters(args.scenarios_per_type, args.total_scenarios, args.shuffle_scenarios, log_names=log_names)) #定义过滤规则（场景数量、日志名称、是否展开等）

    worker = SingleMachineParallelExecutor(use_process_pool=True)
    scenarios = builder.get_scenarios(scenario_filter, worker) #获取符合过滤条件的场景列表
